...
namespace ManagementSystem
{
    public partial class AddEmployee : UserControl
    {
        ...
        private void addEmployee_deleteBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult check = MessageBox.Show("Ви впевнені що хочете видалити " +
                ...
                if (check == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand(updateData, connect))
                        {
                            ...
                            MessageBox.Show("Оновлення прошло успішно!"
        ...
        private void addEmployee_updateBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult check = MessageBox.Show("Ви впевнені що хочете оновити " +
                    "Employee ID: " + addEmployee_id.Text.Trim() + "?", "Confirmation Message"
                    , MessageBoxButtons.YesNo, MessageBoxIcon.Information);
                ...
                if (check == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        string updateData = "UPDATE employees SET full_name = @fullName" +
                            ", gender = @gender, position = @position,contact_number = @contactNum" +
                ...
                if (connect.State == ConnectionState.Closed)
                {
                    try
                    {
                        ...
                        using (SqlCommand checkEm = new SqlCommand(checkEmID, connect))
                        {
                            ...
                            if (count >= 1)
                            {
                                MessageBox.Show(addEmployee_id.Text.Trim() + " вже зайнято"
                                    , "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
                            else
                            {
                                ...
                                string insertData = "INSERT INTO employees " +
                                    "(full_name, gender, position, contact_number" +
                                    ", employee_id, image, salary, insert_date, status) " +
                                    "VALUES(@fullName, @gender, @position, @contactNum" +
                                    ",@employeeID, @image, @salary, @insertDate, @status)";

                                string path = Path.Combine(@"D:\Prog\C#\ManagementSystem\ManagementSystem\Directory\"
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1)
            {
                ...
                addEmployee_position.Text = row.Cells[3].Value.ToString();
                addEmployee_phoneNum.Text = row.Cells[4].Value.ToString();
                addEmployee_id.Text = row.Cells[5].Value.ToString();
